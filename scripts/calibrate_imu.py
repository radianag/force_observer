...
class Imu:
    ...
    def get_avg(self):
        if self.avg_cnt > self.avg_data_pts:
            avg_x = np.sum(self.x_array, axis=0)
            avg_y = np.sum(self.y_array, axis=0)
            print("avg", self.avg_x)
...
if __name__ == '__main__':
    ...
    while (imu.state < num_points ) and not rospy.is_shutdown():
		# print(traj_q1[flat_state[imu.state][0]], traj_q2[flat_state[imu.state][0]])
		# imu.state +=1
		if imu.avg_cnt = 0:
