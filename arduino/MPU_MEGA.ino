...
ros::NodeHandle nh;
std_msgs::Int16MultiArray msg_accel;
//geometry_msgs::Accel msg_accel;

int16_t cnt = 0;
bool if_calibrate = false;
...
void loop()
{
 ...
 /* Serial.print(F("accel x,y,z: "));
  ...
  /*msg_accel.linear.x = accel_t_gyro.value.x_accel;
  msg_accel.linear.y = accel_t_gyro.value.y_accel;
  msg_accel.linear.z = accel_t_gyro.value.z_accel;

  msg_accel.angular.x = accel_t_gyro.value.x_gyro;
  ...
  msg_accel.data = a; 
  msg_accel.data_length = 7;
  
  pa.publish(&msg_accel);
  nh.spinOnce();
