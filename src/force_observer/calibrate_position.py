...
class CalibratePosition:
    ...
    def take_data(self, case_num):
        ...
        if self.data_accel[case_num].shape[0] == 1:
    ...
    def solve_case(self, case_num, threshold=0.1):
        ...
        for i in range(self.data_accel[case_num].shape[0]):
            ...
            if np.linalg.norm(accel) > threshold:
                if case_num == 0:
                    d = mt.sqrt(mt.pow(accel[2], 2) + mt.pow(accel[0], 2))/joint_accel[0]
                elif case_num == 1:
                    d = mt.sqrt(mt.pow(accel[2], 2) + mt.pow(accel[1], 2)) / joint_accel[1]
                else:
